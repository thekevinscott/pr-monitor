...
async function monitor({ github, context, core }) {
    ...
    while (true) {
        ...
        if (inProgressFound.length === 0 && !needsMore)
            break;
        ...
        const parts = [];
        if (needsMore)
            parts.push(`waiting for checks: ${relevantCount}/${MINIMUM_CHECKS}`);
        if (inProgressFound.length > 0)
            parts.push(`in progress: ${JSON.stringify(inProgressFound)}`);
